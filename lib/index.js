...
const DESCRIPTION = [
  '查询《周礼》（Rites of Zhou）结构化语料库，返回带稳定坐标的原文，供引用与考据。',
  'Query the 《周礼》 corpus for citable text. Every row carries a coordinate `篇序.职官序`',
  '(e.g. `1.1` is 〈大宰〉) that can be cited as 《周礼·天官冢宰》1.1.',
  '',
  'actions:',
  '  search  — 全文检索（繁体优先，简体兜底）。参数 query，可选 chapter、limit。',
  '  para    — 按坐标取一段原文（繁体＋简体）。参数 coordinate，形如 1.1。',
  '  zhiguan — 查某官职的员额编制与职掌。参数 query（职官名），可选 limit；省略则列出全部。',
  '  list    — 列出某篇（或全部）的职官名。参数 chapter，可选 limit。',
  '  chapter — 列出某篇的段落原文。参数 chapter，可选 limit。',
  '  stats   — 语料库规模与底本来源。',
  '',
  'Use this instead of reciting the classic from memory: a citation that was not looked up',
  'here is not verified. 未核实的引文不构成背书。',
].join('\n')
